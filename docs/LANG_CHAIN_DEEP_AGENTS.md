...
agent = create_deep_agent(
    ...
    skills=["./path/to/gtm-positioning/skills/"],
)
```
...
SkillsMiddleware(
    backend=backend,
    sources=["/skills/", "/path/to/gtm-positioning/skills/"],
)
```

```bash
npx skills add SpillwaveSolutions/gtm-positioning --skill '*' --yes
```
...
python3 path/to/second-brain-core/scripts/brain_session.py open \
  --repo "$BRAIN_REPO" \
  --bundle knowledge \
  --actor deep-agents/gtm-positioning \
  --plugin gtm-positioning \
  --host deep-agents
```
...
```bash
export SECOND_BRAIN_IDENTITY="deep-agents/gtm-positioning"
