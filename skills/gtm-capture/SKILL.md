...
0. If more than one agent writes the shared brain, open an isolation session (`gtm-session`) and export `SECOND_BRAIN_ROOT`.
   Claim identity `grok-bot/gtm-positioning` (or `deep-agents/gtm-positioning` on Deep Agents).
